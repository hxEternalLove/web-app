 ...
 * Lunar.toLunar(2016-7-6,format); 公历转化农历 format:格式，可传可不传
 * format: oneOf ["ymdw",...]
 */
 var Lunar = {
    MIN_YEAR : 1891,
    ...
    //@param year 阳历年
    //@param between 天数
    lunarByBetween : function(year, between, format, week) {
        ...
        switch (format) {// 返回数据
            case "ymdw":  // 2014 甲午 (马)年 三月 三十 星期日
                lunarArray = year+" "+ this.lunarYear(year)+"（"+this.zodiacYear(year)+"）年 "+m+" "+this.chineseNumber(e)+" 星期"+week;
                break;
            case "md":// 三月 三十
                lunarArray = m + " " + this.chineseNumber(e);
                break;
            case "d":// 三月 三十
                lunarArray = this.chineseNumber(e);
                break;
            default: 
                break;
        }
    ...
    toLunar : function(date, format) {
        ...
        return this.lunarByBetween(year, this.betweenSolarDays(year, month, day, yearData[1], yearData[2]), format, week);
    },
    //转换公历
    //@param year  阴历-年
